...
#define DEFAULT_SERIAL_COM_DI_PIN 1
#define DEFAULT_SERIAL_COM_RO_PIN 3 
...
//#define RS485_RX_PIN_VALUE LOW
    ...
    #define OP_MODE_AUTO_HEAT    0x94
    #define OP_MODE_AUTO_COOL    0x98
...
void ESP32_Midea_RS485Class::begin(HardwareSerial *hwSerial, uint8_t ro_pin, uint8_t di_pin, /*uint8_t re_de_pin,*/ uint8_t master_id, uint8_t slave_id, uint8_t command_time, uint8_t response_timeout)
{
  ...
  SerialBus->begin(MIDEA_BAUDRATE, SERIAL_8N1, 20, 21);
  SlaveId = slave_id;
  MasterId = master_id;
  Master_Send_Time = command_time;
  Slave_Resp_Time = response_timeout;
  ...
  {
    DesiredState.Swing = MIDEA_SWING_ON;
  }
...
uint8_t ESP32_Midea_RS485Class::SetSwing(MideaACOpFeatureSwingType value)
{
...
uint8_t ESP32_Midea_RS485Class::SetSwingPos(MideaACOpFeatureSwingPosType value)
{
   DesiredState.SwingPos = value; 
   DesiredState.Swing=MIDEA_SWING_ON;
   UpdateNextCycle = 1;     
   return 1;     
}
...
void ESP32_Midea_RS485Class::Update()
{
    ...
    }else
    {
      ...
      switch(DesiredState.OpMode)
      {
        ...
        case MIDEA_AC_OPMODE_AUTO_HEAT: SentData[6] =  OP_MODE_AUTO_HEAT; break;
        case MIDEA_AC_OPMODE_AUTO_COOL: SentData[6] =  OP_MODE_AUTO_COOL; break;
      ...
      uint8_t d =  MIDEA_SWING_OFF;//DesiredState.Swing;//71;//ccc++;
      if (DesiredState.Swing == MIDEA_SWING_ON) {
        d= DesiredState.SwingPos;
      }
      //Serial.println(d);
      SentData[9]=d;
      //set timer start
...
uint8_t ESP32_Midea_RS485Class::ParseResponse()
{
    ...
    )
    {
        ...
        switch(ReceivedData[8]) 
        {
            ...
            case OP_MODE_AUTO_HEAT: State.OpMode = MIDEA_AC_OPMODE_AUTO_HEAT; break;
            case OP_MODE_AUTO_COOL: State.OpMode = MIDEA_AC_OPMODE_AUTO_COOL; break;         
